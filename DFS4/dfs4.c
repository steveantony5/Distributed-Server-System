...
status Login()
{
	/***************** Checking user authenticity*********************/
		...
		fp_DFS4_conf = fopen("dfs4.conf","r");
		if(fp_DFS4_conf == NULL)
		{
			printf("Could not open dfs4.conf");
			return ERROR;
		}
		...
		while (fgets(line_from_config, sizeof(line_from_config), fp_DFS4_conf))
		{
			...
			{
				validity = 1;
				printf("Authenticated by DFS4\n");
			}
		}

		if(validity == 0)
			printf("Authentication error on DFS4\n");
