...
status Login()
{
	/***************** Checking user authenticity*********************/
		...
		fp_DFS1_conf = fopen("dfs1.conf","r");
		if(fp_DFS1_conf == NULL)
		{
			printf("Could not open dfs1.conf");
			return ERROR;
		}
		...
		while (fgets(line_from_config, sizeof(line_from_config), fp_DFS1_conf))
		{
			...
			{
				validity = 1;
				printf("Authenticated by DFS1\n");
			}
		}

		if(validity == 0)
			printf("Authentication error on DFS1\n");
